...
def split_message(text: str, max_len: int) -> list[str]:
    ...
    Matches the semantics of the upstream nanobot.utils.helpers.split_message.
    Prefers splitting on newlines to keep paragraphs intact.
    """
    if len(text) <= max_len:
        return [text]
    chunks: list[str] = []
    while text:
        if len(text) <= max_len:
            chunks.append(text)
            break
        # Try to split on the last newline within the budget
        split_at = text.rfind("\n", 0, max_len)
        if split_at <= 0:
            split_at = max_len
...
# ── Local path helpers (replaces upstream nanobot.config.paths) ─────────────────
from pathlib import Path as _Path

def _get_media_dir(channel: str) -> _Path:
    """Return the shared media directory for a given channel."""
    d = _Path.home() / ".nanobot" / "media" / channel
    d.mkdir(parents=True, exist_ok=True)
    return d

def _get_runtime_subdir(name: str) -> _Path:
    """Return a runtime-state subdirectory under ~/.nanobot/."""
    d = _Path.home() / ".nanobot" / name
    d.mkdir(parents=True, exist_ok=True)
    return d
