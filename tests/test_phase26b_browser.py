...
# We import browser module internals
from nanobot.plugins.browser import (
    BrowserTool,
    _check_ssrf,
    _is_safe_js,
    _validate_url,
...
def _setup_browser_with_page(tool: BrowserTool):
    ...
    page = _mock_page()
    tool._browser = AsyncMock()  # Must be AsyncMock for await _browser.close()
...
# Decorator to patch HAS_PLAYWRIGHT for tests needing a mocked browser
...
class TestSSRFProtection:
    ...
    def test_dns_failure(self):
        ...
        with patch(
            "nanobot.plugins.browser.socket.getaddrinfo",
    ...
    async def test_navigate_blocks_internal_ip(self, browser_tool):
        ...
        with patch("nanobot.plugins.browser.socket.getaddrinfo") as mock_dns:
            ...
            result = await browser_tool.execute(
                action="navigate", url="http://evil.com/admin"
            )
...
class TestProgressiveTrust:
    ...
    def test_list_trusted(self, trust_manager):
        ...
        assert "erp.internal.io" in all_domains  # from config
        assert "added.example.com" in all_domains  # runtime
    ...
    async def test_untrusted_domain_auto_trusted(self, browser_tool):
        ...
        with patch(
            "nanobot.plugins.browser._check_ssrf", return_value=(True, "", "93.184.216.34")
        ):
            result = await browser_tool.execute(
                action="navigate", url="https://unknown.example.com"
            )
...
class TestBrowserActions:
    ...
    async def test_click(self, browser_tool):
        """Click action calls page.click with selector."""
        page = _setup_browser_with_page(browser_tool)
        result = await browser_tool.execute(
    ...
    @pytest.mark.asyncio
    async def test_click_no_selector(self, browser_tool):
    ...
    @pytest.mark.asyncio
    async def test_fill(self, browser_tool):
        """Fill action calls page.fill with selector and value."""
        page = _setup_browser_with_page(browser_tool)
        result = await browser_tool.execute(
    ...
    @pytest.mark.asyncio
    async def test_type_text(self, browser_tool):
        """Type action calls page.type with selector and text."""
        page = _setup_browser_with_page(browser_tool)
        result = await browser_tool.execute(
            action="type", selector="#search", text="hello world"
        )
        data = json.loads(result)
        assert data["success"] is True
    ...
    @pytest.mark.asyncio
    async def test_select(self, browser_tool):
    ...
    async def test_screenshot(self, browser_tool):
        ...
        page = _setup_browser_with_page(browser_tool)
        # Config import is local inside _action_screenshot, patch at loader level
        with patch(
            "nanobot.config.loader.get_config",
            side_effect=Exception("no config"),
        ):
            result = await browser_tool.execute(action="screenshot")
            data = json.loads(result)
            assert data["success"] is True
            assert "browser_screenshot_" in data["path"]
    ...
    @pytest.mark.asyncio
    async def test_content_with_selector(self, browser_tool):
        """Content with selector extracts element text."""
        page = _setup_browser_with_page(browser_tool)
        result = await browser_tool.execute(
            action="content", selector="div.main"
        )
        data = json.loads(result)
        assert "Element text" in data["text"]

    @pytest.mark.asyncio
    async def test_content_selector_not_found(self, browser_tool):
        """Content with missing element returns error."""
        page = _setup_browser_with_page(browser_tool)
        page.query_selector = AsyncMock(return_value=None)
        result = await browser_tool.execute(
            action="content", selector="div.nonexistent"
        )
        assert "not found" in result.lower()

    @pytest.mark.asyncio
    async def test_wait_for_selector(self, browser_tool):
        """Wait action waits for element."""
        page = _setup_browser_with_page(browser_tool)
        result = await browser_tool.execute(action="wait", selector="#loaded")
        data = json.loads(result)
        assert data["success"] is True
    ...
    async def test_login_navigates(self, browser_tool):
        ...
        with patch(
            ...
            data = json.loads(result)
            assert data["action"] == "navigate"  # Same as navigate in 26B
            page.goto.assert_called_once()
...
class TestEvaluateWhitelist:
    ...
    def test_query_selector_text_content_allowed(self):
        assert _is_safe_js("document.querySelector('#name').textContent") is True

    def test_query_selector_value_allowed(self):
        assert (
            _is_safe_js("document.querySelector('input.email')?.value") is True
        )

    def test_query_selector_all_length_allowed(self):
        assert (
            _is_safe_js("document.querySelectorAll('li.item').length") is True
        )
    ...
    async def test_evaluate_blocked_returns_error(self, browser_tool):
        ...
        page = _setup_browser_with_page(browser_tool)
        result = await browser_tool.execute(
            action="evaluate", expression="fetch('http://evil.com')"
        )
...
class TestPageLimits:
    ...
    async def test_exceeds_max_pages_reuses_last(self, browser_tool):
        ...
        with patch(
            ...
            result = await browser_tool.execute(
                action="navigate", url="https://example.com/page3"
            )
            # _get_page returns last page, so navigation still works
            data = json.loads(result)
            assert data["action"] == "navigate"
