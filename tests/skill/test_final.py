...
import json
import os
from mail_skill_fixed import process_emails_attachments
...
try:
    ...
    if tool_calls:
        ...
        try:
            ...
            print(f"邮件处理函数调用失败: {e}")
        
except Exception as e:
    print(f"tools参数调用失败: {e}")
    print("尝试备用方案：直接调用邮件处理函数...")
...
print("\n=== 总结 ===")
print("1. 本地LLM函数调用已修复 - 使用tools参数代替functions参数")
print("2. Outlook筛选问题已修复 - 使用手动筛选代替SQL Restrict")
print("3. 完整的邮件处理流程可用：查找邮件→提取附件→分析内容→发送结果")
print("4. 如果函数调用导致模型崩溃，可以直接调用process_emails_attachments()函数")
print("\n=== 测试下一步 ===")
print("1. 确认Outlook连接正常")
print("2. 检查是否有符合条件的邮件")
print("3. 验证附件处理功能")